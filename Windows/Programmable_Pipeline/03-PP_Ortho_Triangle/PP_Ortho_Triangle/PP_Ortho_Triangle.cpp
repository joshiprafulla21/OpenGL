...
int initialize(void)
{
	...
	// PRE-LINKING TO VERTEX ATTRIBUTES
	...
	// POST-LINKING RETRIEVING UNIFORM ALOCATION
	mvpUniform = glGetUniformLocation(gShaderProgramObject, "u_mvp_matrix");

	// MAKE ARRAY OF ALL VALUES OF glVertex() FROM FFP PROGRAM
	const GLfloat triangleVertices[] = { 0.0, 50.0, 0.0,
										-50.0, -50.0, 0.0,
										50.0, -50.0, 0.0 };
...
void display(void)
{
	...
	// DO NECESARY TRANSFORMATION

	// DO NECESARY MATRIX MULTIPLICATION
	modelViewProjectionMatrix = orthographicProjectionMatrix * modelViewMatrix;	// This was done by gluOrtho() in FFP
	
	//SEND NECESSARY MATRICES TO SHADER IN RESPECTIVE UNIFORM
	glUniformMatrix4fv(mvpUniform, 1, GL_FALSE, modelViewProjectionMatrix);

	//BIND WITH vao (this will avoid many repeatitive steps)
	glBindVertexArray(vao);

	//SIMILARLY BIND WITH TEXTURES , IF ANY

	// DRAW NECESSARY SCENES
...
void uninitialize(void)
{
	...
	if (gShaderProgramObject)
	{
		...
		glUseProgram(gShaderProgramObject);
		// ASK PROGRAM HOW MANY SHADERS ATTEACHED TO YOU
		...
		if (pShaders)
		{
			...
			for (shaderNumber = 0; shaderNumber < shaderCount; shaderNumber++)
			{
				...
				glDeleteProgram(pShaders[shaderNumber]);
