...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hprevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("FFP_Deathly_Hallows_Animation!!"),
...
void update(void)
{
	...
	{
		xVal_Triangle = xVal_Triangle - 0.001f;		// Decrease X value of Triangle in glTranslatef()
	}
	if (yVal_Triangle <= 0.0f)
	{
		yVal_Triangle = yVal_Triangle + 0.001f;		// Increase Y value of Triangle in glTranslatef()
	}
	if (xVal_Incircle <= 0.0f)
	{
		xVal_Incircle = xVal_Incircle + 0.001f;		// Increase X value of Incircle in glTranslatef()
	}
	if (yVal_Incircle <= 0.0f)
	{
		yVal_Incircle = yVal_Incircle + 0.001f;		// Increase Y value of Incircle in glTranslatef()
	}

	if (yVal_Line >= 0.0f)
	{
		yVal_Line = yVal_Line - 0.001f;				//Increase X value of Line in glTranslatef()
	}
