...
FILE *gpFile = NULL;
GLdouble dAngle = 0.0;		// angle for rotation of pyramid about Y axis i.e., Spinning

// Following parameters are spesic for light
bool bLight = false;
//Global structure of arrays of light properties
struct Light {
...
/* FOLLOWING PARAMETERS ARE SPECIFIC TO PYRAMID LIGHTING*/
...
/*pYRAMID SPECIFIC PARAMETERS ENDS HERE*/
...
int initialize(void)
{
	...
	glMaterialfv(GL_FRONT, GL_AMBIENT, MaterialAmbient);		//Right
	glMaterialfv(GL_FRONT, GL_DIFFUSE, MaterialDiffuse);		//Right
	glMaterialfv(GL_FRONT, GL_SPECULAR, MaterialSpecular);		//Right
	glMaterialfv(GL_FRONT, GL_SHININESS, MaterialShininess);		//Right
...
void display(void)
{
	...
	glLoadIdentity();
	glTranslatef(0.0f, 0.0f, -4.0f);		//Right
