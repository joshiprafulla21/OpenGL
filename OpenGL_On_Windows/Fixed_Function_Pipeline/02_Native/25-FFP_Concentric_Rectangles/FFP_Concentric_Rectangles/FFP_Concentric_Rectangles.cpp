...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hprevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("FFP_Concentric_Rectangles!!"),
...
void display(void)
{
	...
	glLineWidth(3.0f);
	int count = 0;
	for (float RectSize = 0.0f; RectSize <= 1.0f; RectSize = RectSize + 0.1f)
	{
		count++;
		DrawRectangles(RectSize, count);
	}
...
void DrawRectangles(float RectSize, int count)
{
	...
	{
		glVertex3f(fRadius*cos(angle), fRadius*sin(angle), 0.0f);
	}*/
	//1st Rect

	glVertex3f(1.0f - RectSize, 1.0f - RectSize, 0.0f);
